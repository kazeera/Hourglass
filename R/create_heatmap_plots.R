#' Create heatmap plots helper in a folder in streamlined approach
#'
#' Run heatmap analysis helper
#'
#' @inheritParams run_comparisons ds,rowAnns,colAnns,out_dir,labels,manyNA_anns
...
create_heatmap_plots <- function(ds, rowAnns = 1, colAnns = NA, out_dir = ".", labels = "Sub1") {
  ...
  if (!any(is.na(ds$vals))) { # grepl("imputed", ds$name)){
    # Run all cores with NAs, unclustered
    tryCatch(
      {
...
#' @param pal A named character vector of colors (R or hex), where the names are the groups in row annotations and column annotations
...
create_heatmap_plots_helper <- function(ds, rowAnns = 1, colAnns = NA, out_dir = ".", labels = "", pal = NA, clust_row = F, clust_col = F,
    ...
    tryCatch(
      {
        ...
        df_FCp <- make_FC.pval_df(df, rowAnn_col = 1)
        # Make plot
        make_FC.pval_plot(df_FCp, x_lab = rowAnns[1], y_lab = colAnns[2], plot_title = labels, out_dir = out_dir, save.to.file = T)
      },
      error = function(err) {
        print(sprintf("%s", err))
      }
  ...
  if (make.corrplot) {
    tryCatch(
      {
        run_corrplot_analysis(df, out_dir = out_dir, labels = labels)
      },
      error = function(err) {
        print(sprintf("%s", err))
      }
    ...
    tryCatch(
      {
        ...
        # All rows
        plot_overview_corr_scatt(df[, -1], out_dir, c(labels, "All"), corr_method = "spearman", save.to.file = F)
        # Plot for each group
        for (group in unique(df[, 1])) {
          if (is.na(group)) next
          df3 <- df[df[, 1] == group, -1]
          plot_overview_corr_scatt(df3, out_dir, c(labels, group), corr_method = "spearman", save.to.file = F)
        }
      },
      error = function(err) {
        print(sprintf("%s", err))
    ...
    tryCatch(
      {
        ...
        plot_indiv_corrscatt(df[, -1], out_dir, labels)
      },
      error = function(err) {
        print(sprintf("%s", err))
      }
  ...
  if (make.barplot) {
    tryCatch(
      {
        run_profile_barplot(df, rowAnn_col = 1, out_dir, labels)
      },
      error = function(err) {
        print(sprintf("%s", err))
      }
  ...
  if (make.indiv.boxplot) {
    ...
    pdf_filename <- sprintf("%s/%s_indiv_boxplots.pdf", out_dir, paste(labels, collapse = "_"))
    # Create pdf file of all plots
